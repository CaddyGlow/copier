...
Downloads the latest copier release from GitHub and installs the binary for the
...
prompt_overwrite() {
  ...
  if [[ -n "${existing_version}" && "${latest_tag}" != "unknown" ]]; then
    ...
    if [[ "${existing_clean}" == "${latest_clean}" ]]; then
      prompt="Reinstall copier ${latest_tag}? [y/N] "
    else
      prompt="Replace copier ${existing_version} with ${latest_tag}? [y/N] "
    fi
  fi
...
check_existing_install() {
  ...
  echo "Found existing copier at ${install_path}"
